 ...
 */
    ...
    public class DMOJAFK {  // actually wlmoj minecraft solution but sureasdasdasdasdasdasdasdasd
        ...
        public static void main(String[] args) throws IOException {
            ...
            for (int k = 0; k < t; k++) {
                ...
                int n1 = readInt();
                boolean[][] visited = new boolean[n1 + 1][n + 1];    // najes a new boolean array so we dont fuck up our bfs
                int startx = 0, starty = 0, endx = 0, endy = 0;     // stores the row num and col num for start and ending points
                for (int i = 1; i <= n1; i++) {  //  goes through each line
                    String s = readLine();  // reads the line
                    for (int j = 1; j <= n; j++) {  // goes through the characters in each line
                        if (s.charAt(j - 1) == 'X') {   // checks if it is a wall
                            visited[i][j] = true;       // if its a wall it sets that place to true so we dont visit it during bfs
                        }
                        if (s.charAt(j - 1) == 'C') {   // checks if it is starting point- if so it marks the two values to the indicies
                            startx = i;
                            starty = j;
                        }
                        if (s.charAt(j - 1) == 'W') {   // same procedure as above for the ending point
                            endx = i;
                            endy = j;
                        }
                    }
                ...
                int distance[][] = new int[n1 + 1][n + 1];       // creates a new 2d distance array
                ...
                while (!q.isEmpty()) {  //blah blah regular bfs declaration
                    ...
                    if (cur < n1) { // down moving this time  MOOOOOVING
                ...
                if (!visited[endx][endy]||distance[endx][endy]>=60) System.out.println("#notworth");
                else {
